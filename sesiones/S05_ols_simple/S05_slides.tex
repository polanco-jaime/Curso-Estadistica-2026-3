...
\begin{itemize}
  \item ¿Qué proporción de estudiantes de Ciencias Econ\'omicas alcanza nivel B1 en inglés?
...
¿Qu\'e proporci\'on del sal\'on usa iPhone vs Android? Si encuestas a 30 compa\~neros y 18 dicen iPhone, $\hat{p} = 18/30 = 0.60$. Pero, ¿qu\'e tan preciso es ese 60\%? Para eso necesitamos un \alert{intervalo de confianza}.
\end{exampleblock}
\end{frame}

\begin{frame}{Motivación: Estimar Proporciones (cont.)}
...
\textbf{Ejemplo}: En una muestra de $n = 200$ estudiantes de Ciencias Econ\'omicas, 140 superan B1 en inglés.
\[
\hat{p} = \frac{140}{200} = 0.70 = 70\%
\]

{\small\textbf{Conexión con Bernoulli}: Cada estudiante es un ensayo Bernoulli con probabilidad $p$ de éxito. Entonces $x \sim \text{Binomial}(n, p)$.}
\end{frame}
...
Estamos 95\% confiados de que la proporción poblacional de estudiantes de Ciencias Econ\'omicas que superan el nivel B1 en inglés está entre 68\% y 76\%.

\smallskip
\textit{Pensemos en el ejemplo del sal\'on}: si 18 de 30 usan iPhone ($\hat{p} = 0.60$), el IC al 95\% ser\'ia aproximadamente $(0.42, 0.78)$. Es decir, la verdadera proporci\'on de usuarios de iPhone en la uni podr\'ia estar entre 42\% y 78\%.
\end{exampleblock}
...
# Ordenar por p_hat
ic_prop_depto <- ic_prop_depto %>% arrange(desc(p_hat))
...
ggplot(ic_prop_depto,
  ...
  labs(x = "Proporcion que supera B1 (IC 95%)",
       y = "Departamento",
       title = "Proporcion Estudiantes Ciencias Economicas Superan B1 en Ingles") +
  theme_minimal()
\end{lstlisting}
\end{frame}
...
\textbf{Aplicaciones}:
\begin{itemize}
  \item Encuesta de satisfacción: ¿cuántos estudiantes de Ciencias Econ\'omicas encuestar para estimar el salario inicial promedio con error $\pm \$300{,}000$ COP?
...
¿Cu\'antas personas necesitas encuestar para saber si m\'as del 50\% de la uni prefiere Spotify a Apple Music, con un margen de error de $\pm 5\%$?
\end{exampleblock}
\end{frame}

\begin{frame}{Planificación del Muestreo (cont.)}
...
Recordemos que el margen de error es $E = z_{\alpha/2} \frac{\sigma}{\sqrt{n}}$. Despejando $n$:
...
\small
\textbf{Escenario}: Queremos estimar la media de \variable{MOD\_INGLES\_PUNT} en Ciencias Econ\'omicas con un margen de error de $\pm 2$ puntos al 95\% de confianza. De estudios previos, $\sigma \approx 18$.
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Si queremos $E = 1$ en lugar de $E = 2$, entonces $n$ se multiplica por $2^2 = 4$ \quad ($n \propto 1/E^2$)
  \end{itemize}

  \item \textbf{Mayor confianza} $\Rightarrow$ mayor $n$
  \begin{itemize}
    \item 95\%: $z = 1.96$ \quad vs \quad 99\%: $z = 2.576$ ($+31\%$)
    \item $n$ aumenta en $(2.576/1.96)^2 = 1.73$, es decir, 73\% más
  \end{itemize}

  \item \textbf{Mayor variabilidad ($\sigma$)} $\Rightarrow$ mayor $n$ \quad ($n \propto \sigma^2$)
\end{enumerate}
\end{frame}
...
\textbf{Opciones}:
\begin{enumerate}
  \item \textbf{Estudio piloto}: Muestra pequeña ($n_{\text{piloto}} = 30\text{-}50$), calcular $s$ y usar como estimación de $\sigma$.
  \item \textbf{Literatura}: Buscar estudios previos similares y usar su $\sigma$ reportada.
  \item \textbf{Rango}: Si conocemos el rango aproximado, usar $\sigma \approx \text{rango}/4$ o $\text{rango}/6$.
\end{enumerate}
...
\small
\textbf{Escenario}: Queremos estimar la proporción de estudiantes de Ciencias Econ\'omicas que superan B2 en inglés (\variable{MOD\_INGLES\_PUNT} $\geq 180$) con margen de error $\pm 3\%$ al 95\%.

\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
# Parametros
E <- 0.03; confianza <- 0.95
...
Si tienes una estimación razonable de $p$ (de un piloto o literatura), \'usala. Si no, usa $p^* = 0.5$ para estar seguro.

\smallskip
\textit{Volviendo a Spotify vs Apple Music}: si no tienes idea de la preferencia, usa $p^* = 0.5$. Si ya sabes que en tu facultad $\approx 70\%$ usa Spotify, usa $p^* = 0.70$ y necesitar\'as menos encuestados.
\end{exampleblock}
\end{frame}
...
\vspace{0.2cm}
\textbf{Ejemplo}: Seleccionar 500 estudiantes de Ciencias Econ\'omicas al azar de toda Colombia.
...
\begin{itemize}
  \item Departamento o región \quad\quad \textbullet\ Tipo de IES (pública/privada)
  \item Nivel socioeconómico \quad\quad \textbullet\ Género
\end{itemize}
\end{block}
\end{frame}
...
\vspace{0.3cm}
\textbf{Ejemplo}: Si el 15\% de los estudiantes de Ciencias Econ\'omicas están en Antioquia, asignamos el 15\% de la muestra a Antioquia.
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
# Supongamos que queremos n = 1000 estudiantes de Ciencias Economicas
...
\begin{enumerate}
  \item Calcular IC al 95\% para la proporción de estudiantes de Ciencias Econ\'omicas que superan B2 en inglés (\variable{MOD\_INGLES\_PUNT} $\geq 180$)
