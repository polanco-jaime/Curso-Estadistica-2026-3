...
ggplot(ic_prop_depto,
  ...
  labs(x = "Proporcion que supera B1 (IC 95%)",
       y = "Departamento",
       title = "Proporcion Estudiantes NI Superan B1 en Ingles") +
  theme_minimal()
\end{lstlisting}
\end{frame}
...
\small
\textbf{Escenario}: Queremos estimar la media de \variable{MOD\_INGLES\_PUNT} en NI con un margen de error de $\pm 2$ puntos al 95\% de confianza. De estudios previos, $\sigma \approx 18$.
...
\small
\textbf{Escenario}: Queremos estimar la proporción de estudiantes de NI que superan B2 en inglés (\variable{MOD\_INGLES\_PUNT} $\geq 180$) con margen de error $\pm 3\%$ al 95\%.
