...
¿Cómo se relaciona el nivel de inglés MCER (A-, A1, A2, B1, B+) con el tipo de IES (pública/privada) entre estudiantes de Ciencias Econ\'omicas y Administrativas?
\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{Frecuencias marginales}
...
\begin{frame}[fragile]{Frecuencias relativas}
\begin{lstlisting}
...
\vspace{0.2cm}
\textbf{Cada tipo de proporción responde una pregunta diferente:}
\begin{itemize}
  \item Total: ¿qué porcentaje del total cae en esta celda?
  \item Por fila: ¿cómo se distribuye cada nivel MCER entre tipos de IES?
  \item Por columna: ¿cómo se distribuye cada tipo de IES entre niveles MCER?
\end{itemize}
\end{frame}
...
\begin{block}{Observaci\'on visual}
Parece haber una diferencia: las IES privadas tienen mayor proporci\'on en B1 y B+. ?`Es estad\'isticamente significativa?
\end{block}
\end{frame}
...
?`Hay asociaci\'on entre g\'enero y preferencia de red social (TikTok vs Instagram vs YouTube)? Si las variables fueran independientes, hombres y mujeres elegir\'ian las redes en la misma proporci\'on.
\end{exampleblock}
\end{frame}
...
\begin{frame}{Frecuencias esperadas: ejemplo}
\textbf{Ejemplo:} celda (A2, privada)
\[
E_{\text{A2,privada}} = \frac{1648 \times 2750}{5085} = \frac{4{,}532{,}000}{5085} \approx 891.24
\]
...
\begin{block}{Lógica}
\begin{itemize}
  \item $R_i = 1648$ (total de estudiantes con nivel A2)
  \item $C_j = 2750$ (total de estudiantes en IES privadas)
  \item $n = 5085$ (total general)
\end{itemize}
Si las variables fueran independientes, la proporción de A2 sería la misma en ambos tipos de IES.
\end{block}
\end{frame}

\begin{frame}{El estad\'istico $\chi^2$}
...
\]
\vspace{-0.2cm}
donde $O_{ij}$ = frec.\ observada, $E_{ij}$ = frec.\ esperada, $r$ = filas, $c$ = columnas.
\end{block}
...
\begin{itemize}
  \item $\chi^2 = 186.42$, gl $= 4$, p-valor $< 2.2 \times 10^{-16}$ (prácticamente 0)
\end{itemize}
\end{block}
...
\]
donde $\chi^2$ = estadístico chi-cuadrado, $n$ = tamaño muestral, $r$ = filas, $c$ = columnas.
\end{block}
...
\begin{frame}{V de Cramér: cálculo manual}
...
\textbf{Datos usados:}
\begin{itemize}
  \item $\chi^2 = 186.42$ (del resultado de \code{chisq.test()})
  \item $n = 5085$ (total de observaciones)
  \item $\min(r, c) - 1 = \min(5, 2) - 1 = 1$
\end{itemize}
\end{frame}
...
\begin{frame}{?`Qu\'e es bondad de ajuste?}
\begin{block}{Pregunta diferente}
En la prueba de \textbf{independencia}, comparamos dos variables categ\'oricas observadas.
En la prueba de \textbf{bondad de ajuste}, comparamos una variable observada con una distribuci\'on te\'orica esperada.
\end{block}

\vspace{0.2cm}
\textbf{Hip\'otesis:}
\begin{itemize}
  \item $H_0$: Los datos provienen de la distribuci\'on especificada
  \item $H_1$: Los datos NO provienen de esa distribuci\'on
\end{itemize}
\end{frame}

\begin{frame}{Bondad de ajuste: ejemplos}
\begin{exampleblock}{Ejemplos de preguntas}
\begin{itemize}
  \item ?`Los niveles MCER se distribuyen uniformemente?
  \item ?`El puntaje de ingl\'es sigue una distribuci\'on normal?
  \item ?`La proporci\'on de estudiantes por regi\'on coincide con la distribuci\'on poblacional de Colombia?
  \item ?`Los sabores de helado se venden igual o hay preferencias? (Si $H_0$: distribuci\'on uniforme, cada sabor deber\'ia venderse en igual proporci\'on.)
\end{itemize}
\end{exampleblock}
\end{frame}

\begin{frame}{Estad\'istico de bondad de ajuste}
\begin{block}{F\'ormula}
\[
\chi^2 = \sum_{i=1}^{k} \frac{(O_i - E_i)^2}{E_i}
\]
\vspace{-0.2cm}
donde $O_i$ = frec.\ observada en la categor\'ia $i$, $E_i$ = frec.\ esperada (seg\'un la distribuci\'on te\'orica), $k$ = n\'umero de categor\'ias.
\end{block}

\vspace{0.1cm}
\textbf{Grados de libertad:}
$\text{gl} = k - p - 1$, donde $p$ = n\'umero de par\'ametros estimados de los datos.
\begin{itemize}
  \item Distribuci\'on te\'orica completamente especificada: $p = 0$
  \item Si estimamos $\mu$ y $\sigma$ de los datos: $p = 2$
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Frecuencias esperadas bajo normalidad}
\begin{lstlisting}
# Frecuencias observadas
...
\begin{frame}[fragile]{Prueba de bondad de ajuste: normalidad}
\begin{lstlisting}
...
\vspace{0.2cm}
\begin{alertblock}{Nota sobre los grados de libertad}
gl $= k - p - 1 = 10 - 2 - 1 = 7$, donde $p = 2$ porque estimamos $\mu$ y $\sigma$ de los datos.
\end{alertblock}
\end{frame}
...
p-valor $< 0.001$ $\Rightarrow$ rechazamos $H_0$: los puntajes de inglés NO siguen una distribución normal.
\end{alertblock}
\end{frame}
...
\textbf{Pregunta:} ¿La distribución regional de estudiantes de Ciencias Econ\'omicas refleja la distribución poblacional de Colombia?
...
\textbf{Conclusión:} La distribución regional de estudiantes de Ciencias Econ\'omicas NO refleja la distribución poblacional (posiblemente por concentración de programas en ciertas regiones).
\end{frame}
...
\begin{tikzpicture}[node distance=1.2cm, auto,
                    block/.style={rectangle, draw, fill=blue!10,
                                  text width=4.5em, align=center,
                                  rounded corners, minimum height=2.2em,
                                  font=\small},
                    decision/.style={diamond, draw, fill=orange!10,
                                     text width=3.5em, align=center,
                                     inner sep=0pt, font=\small}]

\node [decision] (tipo) {Tipo de variable};
\node [block, below left=0.8cm and 1cm of tipo] (num) {Numérica};
\node [block, below right=0.8cm and 1cm of tipo] (cat) {Categórica};

\node [decision, below=0.6cm of num] (ngrupos) {¿Cuántos grupos?};
\node [block, below left=0.6cm and 0.3cm of ngrupos] (ttest) {1 o 2:\\ t-test};
\node [block, below right=0.6cm and 0.3cm of ngrupos] (anova) {3+:\\ ANOVA};

\node [decision, below=0.6cm of cat] (pregunta) {¿Qué pregunta?};
\node [block, below left=0.6cm and 0.3cm of pregunta] (indep) {Asociación:\\ $\chi^2$ indep.};
\node [block, below right=0.6cm and 0.3cm of pregunta] (bondad) {Bondad:\\ $\chi^2$ ajuste};
...
\begin{itemize}
  \item Compara dos variables categóricas --- $H_0$: son independientes
  \item $\chi^2 = \sum \frac{(O_{ij} - E_{ij})^2}{E_{ij}}$, \; gl $= (r-1)(c-1)$
...
\begin{itemize}
  \item Compara datos observados con una distribución teórica
  \item $H_0$: los datos siguen la distribución especificada
  \item $\chi^2 = \sum \frac{(O_i - E_i)^2}{E_i}$, \; gl $= k - p - 1$
  \item Alternativa para normalidad: Shapiro-Wilk
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
  \item Estadístico de prueba, grados de libertad y p-valor
...
\begin{exampleblock}{Siguiente tema}
