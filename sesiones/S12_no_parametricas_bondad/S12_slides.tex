...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw=javeriana, fill=javeriana!10, thick, minimum width=2.2cm, minimum height=0.6cm, rounded corners, font=\small},
...
\begin{frame}{El pipeline estadístico: proceso iterativo}
\begin{block}{El an\'alisis estad\'istico es un proceso iterativo}
\begin{itemize}
\item Cada paso informa el siguiente
\item Los diagn\'osticos pueden llevarnos a re-especificar el modelo
...
\begin{exampleblock}{Analog\'ia: es como cocinar}
\small
Primero los ingredientes (datos), luego la preparaci\'on (limpieza), la cocci\'on (modelo), y la presentaci\'on del plato (conclusiones). Si queman algo en la cocci\'on (diagn\'osticos fallan), toca volver atr\'as y ajustar.
\end{exampleblock}
\end{frame}
...
\begin{alertblock}{Mal an\'alisis}
\begin{itemize}
\item Saltar directamente a regresi\'on
\item No explorar los datos
\item Ignorar supuestos
\item No verificar diagn\'osticos
\item Reportar solo p-valores
\end{itemize}

\vspace{0.1cm}
\small
Es como publicar un TikTok sin editar: sale cualquier cosa.
\end{alertblock}
\end{column}

\begin{column}{0.48\textwidth}
\begin{exampleblock}{Buen an\'alisis}
\begin{itemize}
...
\vspace{0.1cm}
\small
Es como un video bien producido: gui\'on, grabaci\'on, edici\'on, revisi\'on y publicaci\'on.
\end{exampleblock}
\end{column}
\end{columns}
...
\vspace{0.3cm}
\begin{center}
\textbf{Objetivo de hoy}: aplicar el pipeline completo a datos ICFES de Ciencias Econ\'omicas y Administrativas.
\end{center}
\end{frame}

\begin{frame}{Caso de estudio: competencia en inglés en Ciencias Econ\'omicas}
\begin{block}{Contexto}
Los programas de Ciencias Econ\'omicas y Administrativas requieren dominio del inglés como competencia clave. El examen Saber Pro evalúa esta competencia con la prueba de inglés (MOD\_INGLES\_PUNT, escala 0-300, convertible a niveles MCER: A-, A1, A2, B1, B2).
\end{block}
...
\begin{center}
\large
\textcolor{javeriana}{?`Qu\'e factores socioecon\'omicos y acad\'emicos determinan\\el desempe\~no en ingl\'es de estudiantes de Ciencias Econ\'omicas y Administrativas\\en Colombia?}
\end{center}

\vspace{0.2cm}
\textbf{Relevancia:}
\begin{itemize}
\item Informar pol\'iticas de admisi\'on y nivelaci\'on
\item Identificar grupos vulnerables que requieren apoyo
\item Evaluar la equidad en el acceso a formaci\'on en ingl\'es
\end{itemize}
\end{frame}

% ============================================================================
\section{Paso 1: Pregunta de investigaci\'on}
% ============================================================================

\begin{frame}{Formulando una buena pregunta de investigación}
\begin{block}{Caracter\'isticas de una buena pregunta}
\begin{itemize}\setlength{\itemsep}{0pt}
\item \textbf{Clara}: t\'erminos bien definidos
\item \textbf{Medible}: variables observables
\item \textbf{Relevante}: tiene implicaciones pr\'acticas o te\'oricas
\item \textbf{Acotada}: no demasiado amplia ni demasiado estrecha
\end{itemize}
\end{block}

\small
Recuerden la sesi\'on de descriptiva (S01): primero hay que saber \textit{qu\'e} queremos medir antes de calcular medias y desviaciones. La pregunta es el GPS de todo el an\'alisis.
\end{frame}

\begin{frame}{Formulando una buena pregunta: ejemplo}
\begin{columns}
\begin{column}{0.48\textwidth}
...
"¿Cuál es el efecto del estrato socioeconómico y del tipo de IES en el puntaje de inglés en Saber Pro para estudiantes de Ciencias Econ\'omicas?"
...
\item Población: estudiantes Ciencias Econ\'omicas
\end{itemize}
\end{exampleblock}
\end{column}
\end{columns}
...
\begin{frame}[fragile]{Cargando los datos: output}
...
# Filtrar solo estudiantes de Ciencias Economicas
ni_data <- icfes %>%
...
\begin{frame}[fragile]{Creando variable REGION}
\begin{lstlisting}
...
\begin{frame}{Valores faltantes: consideraciones}
\begin{alertblock}{Importante}
\begin{itemize}
\item \code{na.omit()} elimina TODAS las filas con alg\'un NA (puede ser demasiado dr\'astico)
\item Mejor: eliminar NA solo en variables clave
\item Reportar cu\'antas observaciones se perdieron y por qu\'e
\end{itemize}

\small
Es como limpiar su playlist de Spotify: no borren todo, solo las canciones que realmente no sirven. Cada dato que eliminan es informaci\'on que pierden.
\end{alertblock}
\end{frame}
...
\footnotesize
\textbf{Siempre verificar:} rangos v\'alidos, categor\'ias correctas, no haya valores imposibles.
\end{frame}
...
\begin{frame}[fragile]{EDA: interpretación de descriptivos}
\begin{exampleblock}{Output esperado}
\small
\begin{verbatim}
...
\begin{frame}[fragile]{EDA: desagregación por estrato y tipo de IES}
\begin{lstlisting}
...
\begin{frame}[fragile]{EDA: desagregación por región}
\begin{lstlisting}
# Por region
ni_data %>%
  group_by(REGION) %>%
...
\textbf{Observaciones:}
\begin{itemize}\setlength{\itemsep}{0pt}
\item Relaci\'on positiva entre estrato y puntaje (gradiente claro)
\item La variabilidad (DE) disminuye al aumentar el estrato
\item Estudiantes de estrato 6 punt\'uan 40 puntos m\'as que estrato 1 (diferencia sustantiva)
\end{itemize}
\end{frame}

\begin{frame}{EDA: hallazgos preliminares (cont.)}
\textcolor{javeriana}{Pregunta}: ?`Esta diferencia es estad\'isticamente significativa? $\to$ necesitamos pruebas formales.

\vspace{0.3cm}
Recuerden S03 (muestreo): estas son estad\'isticas \textit{muestrales} --- necesitamos IC y pruebas para saber si reflejan la poblaci\'on.
\end{frame}
...
ggplot(ni_data, aes(x = MOD_INGLES_PUNT)) +
  ...
  labs(title = "Distribucion del puntaje de ingles en Ciencias Economicas",
...
\begin{frame}[fragile]{EDA: scatterplot Saber 11 vs Saber Pro}
\begin{lstlisting}
...
\begin{frame}[fragile]{EDA: proporción que alcanza B1}
\begin{lstlisting}
...
\begin{frame}[fragile]{IC para la media por región: resultados}
\begin{exampleblock}{Output esperado}
\small
\begin{verbatim}
...
\begin{frame}[fragile]{IC para proporciones: resultados}
\begin{exampleblock}{Output esperado}
\small
\begin{verbatim}
...
\begin{frame}{Prueba t: conclusión}
\textbf{Conclusión:}
\begin{itemize}
...
\begin{frame}[fragile]{Chi-cuadrado: resultados}
...
\begin{frame}{Tamaño del efecto: interpretación}
\textbf{Interpretación:}
\begin{itemize}
\item $d = 0.62$ (efecto mediano: 0.2 = pequeño, 0.5 = mediano, 0.8 = grande)
\item La diferencia pública-privada es significativa Y sustantiva
\item El tamaño del efecto cuantifica la relevancia práctica
\end{itemize}
\end{frame}
...
\begin{frame}{Modelo múltiple: conclusiones}
\textbf{Conclusiones:}
\begin{itemize}
...
\begin{frame}{OVB: explicación del cambio}
\textbf{?`Por qu\'e cambia el coeficiente de estrato?}
...
\item El efecto causal verdadero del estrato es \textasciitilde{}2.1 puntos (controlado)
\end{itemize}

\small
Recuerden S06 (OLS m\'ultiple): controlar variables es como aislar el efecto de un solo ingrediente en una receta --- hay que mantener todo lo dem\'as constante para saber qu\'e aporta cada uno.
\end{frame}
...
\item Soluci\'on: usar errores est\'andar robustos
\end{itemize}
\end{frame}
...
\footnotesize
\textbf{Acci\'on}: si hay observaciones con $D_i > 4/n$, investigar (?`error de datos? ?`caso especial?). Si son leg\'itimas, reportar an\'alisis de sensibilidad.
\end{frame}
...
\begin{enumerate}
\item \textbf{Desempeño general}: El puntaje promedio de inglés en Ciencias Econ\'omicas es 154.3 (DE = 18.7), equivalente a nivel A2/B1 en el MCER.
...
\item \textbf{Persistencia del desempe\~no}: El puntaje en Saber 11 es el predictor m\'as fuerte ($\beta = 0.88$, $t = 25.8$). Las brechas vienen desde el colegio --- como cuando el algoritmo de una red social te encasilla desde el principio.
...
\item \textbf{Generalización}: Los resultados son específicos para Ciencias Econ\'omicas en Colombia en 2023. Pueden no aplicar a otros programas o países.
\end{itemize}
\end{block}
\end{frame}

\begin{frame}{Futuras investigaciones}
\begin{exampleblock}{Líneas de investigación futura}
\begin{itemize}
...
\item \textbf{Interpretación contextualizada}: Conectar resultados estadísticos con implicaciones para Ciencias Econ\'omicas (ej: "Esta brecha equivale a medio nivel MCER").
...
\footnotesize
\textcolor{javeriana}{Recordatorio}: El c\'odigo es parte del informe. Debe ser legible y reproducible.
\end{frame}

\begin{frame}{R\'ubrica para el proyecto final}
\footnotesize
\begin{center}
\renewcommand{\arraystretch}{1.1}
...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw=javeriana, fill=javeriana!10, thick, minimum width=3cm, minimum height=0.5cm, rounded corners, font=\small}
]
...
\item \textbf{Verificar supuestos}: OLS sin diagn\'osticos es como postear sin revisar --- puede salir muy mal.
...
\item Hoja de f\'ormulas (draft para el examen)
\item Calculadora
\end{itemize}
\end{alertblock}
\end{frame}
