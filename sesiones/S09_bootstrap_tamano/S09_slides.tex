...
\vspace{0.1cm}
\begin{exampleblock}{Pregunta de investigaci\'on}
\small \textquestiondown Podemos \textbf{predecir} el desempe\~no en ingl\'es en Saber Pro a partir del puntaje de ingl\'es en Saber 11? Necesitamos un \textbf{modelo de regresi\'on}.

\smallskip
\textbf{Analog\'ia:} Imaginen que quieren predecir su nota del parcial a partir de las horas de estudio. La regresi\'on es la herramienta que les permite hacer esa predicci\'on de forma sistem\'atica.
\end{exampleblock}
\end{frame}
...
\end{block}
\vspace{-2pt}
{\small \textbf{Interpretaci\'on:} $\beta_0$ y $\beta_1$ son \textbf{par\'ametros poblacionales} (desconocidos). Los estimaremos con $\hat{\beta}_0$ y $\hat{\beta}_1$ usando los datos.}
\end{frame}
...
\begin{lstlisting}
# Cargar datos ICFES para Ciencias Economicas
...
\begin{frame}{M\'etodo de M\'inimos Cuadrados Ordinarios (OLS)}
\begin{alertblock}{Definici\'on}
El m\'etodo \textbf{OLS} (Ordinary Least Squares) elige $\hat{\beta}_0$ y $\hat{\beta}_1$ que minimizan:
\[
...
\vspace{0.3cm}
En otras palabras: OLS busca la l\'inea que minimiza los errores de predicci\'on al cuadrado --- la l\'inea que mejor ``le pega'' a los datos.
\end{frame}
...
\begin{frame}{Relaci\'on con la correlaci\'on}
El estimador OLS de la pendiente se puede escribir como:
\[
\hat{\beta}_1 = r \times \frac{s_Y}{s_X}
\]
\begin{small}
donde $r$ = correlaci\'on de Pearson, $s_Y$ = desviaci\'on est\'andar de $Y$, $s_X$ = desviaci\'on est\'andar de $X$.
\end{small}

\vspace{0.2cm}
\textbf{Interpretaci\'on:}
\begin{itemize}
  \item Si $r = 0$ $\Rightarrow$ $\hat{\beta}_1 = 0$ (no hay relaci\'on lineal)
  \item Si $s_X = s_Y$ y $r = 1$ $\Rightarrow$ $\hat{\beta}_1 = 1$ (relaci\'on 1 a 1)
  \item El signo de $\hat{\beta}_1$ es el mismo que el de $r$
\end{itemize}

\vspace{0.1cm}
\begin{exampleblock}{Nota}
\small Aunque $\hat{\beta}_1$ depende de $r$, la regresi\'on aporta m\'as informaci\'on: cuantifica el cambio esperado en $Y$ por unidad de cambio en $X$. Por ejemplo, no solo sabemos que horas de estudio y nota est\'an correlacionadas, sino que cada hora adicional sube la nota en $\hat{\beta}_1$ puntos.
\end{exampleblock}
\end{frame}

\begin{frame}[fragile]{Aplicaci\'on en R: \texttt{lm()}}
...
{\small \textbf{Propiedades OLS:} $\sum e_i = 0$ y $\sum X_i e_i = 0$ (ortogonalidad).}
\end{frame}
...
\small $\Rightarrow$ Inferencia (pruebas t, IC) no válida en muestras pequeñas. Solución: bootstrap, transformaciones, o confiar en TCL si $n$ es grande.
\end{alertblock}
\end{frame}
...
\begin{frame}{Coeficiente de determinaci\'on $R^2$}
...
\textbf{Interpretaci\'on:}
\begin{itemize}
  \item $R^2$ = proporci\'on de la variabilidad de $Y$ \textbf{explicada} por $X$
  \item Rango: $R^2 \in [0, 1]$
  \item $R^2 = 0$ $\Rightarrow$ $X$ no explica nada de $Y$ (SSR = 0)
  \item $R^2 = 1$ $\Rightarrow$ ajuste perfecto (SSE = 0, todos los puntos sobre la l\'inea)
\end{itemize}

\vspace{0.2cm}
\begin{exampleblock}{Ejemplo}
Si $R^2 = 0.65$, el 65\% de la variabilidad del puntaje de ingl\'es en Saber Pro se explica por el puntaje en Saber 11. El 35\% restante se debe a otros factores.
\end{exampleblock}
\end{frame}

\begin{frame}{Coeficiente de determinaci\'on $R^2$ (cont.)}
\begin{exampleblock}{Analog\'ia: $R^2$ en su vida}
\textquestiondown Qu\'e porcentaje de la variaci\'on en sus notas se explica por horas de estudio?
\begin{itemize}
  \item Si $R^2 = 0.6$, el 60\% de las diferencias en notas se explican por cu\'anto estudian.
  \item El 40\% restante depende de otros factores (sue\~no, m\'etodo de estudio, etc.).
\end{itemize}
\end{exampleblock}

\vspace{0.3cm}
\begin{alertblock}{Nota importante}
$R^2$ es siempre positivo (es un cuadrado), pero $r$ puede ser negativo. El signo de la relaci\'on se ve en $\hat{\beta}_1$ (o en $r$), no en $R^2$.
\end{alertblock}
\end{frame}

\begin{frame}{Relaci\'on entre $R^2$ y correlaci\'on}
En regresi\'on lineal simple (un solo predictor), se cumple:
\[
R^2 = r^2
\]
donde $r$ es el coeficiente de correlaci\'on de Pearson entre $X$ e $Y$.
...
\]
donde $n - 2$ son los grados de libertad ($n$ observaciones menos 2 par\'ametros estimados).
\end{block}
...
\vspace{0.1cm}
La ra\'iz cuadrada de MSE se llama \textbf{error est\'andar de la regresi\'on}: $s = \sqrt{\text{MSE}}$ = residual standard error.

\vspace{0.1cm}
\begin{exampleblock}{Interpretaci\'on}
\small $s$ representa la desviaci\'on est\'andar t\'ipica de los residuos. Si $s = 12$, en promedio los valores observados se desv\'ian $\pm 12$ puntos de la l\'inea de regresi\'on.
\end{exampleblock}
\end{frame}
...
\begin{block}{Hip\'otesis m\'as com\'un}
\begin{itemize}
  \item $H_0: \beta_1 = 0$ (no hay relaci\'on lineal entre $X$ e $Y$)
  \item $H_1: \beta_1 \neq 0$ (existe relaci\'on lineal)
\end{itemize}
\end{block}

\vspace{0.1cm}
\textbf{Estad\'istico de prueba:}
$t = \hat{\beta}_1 / \text{SE}(\hat{\beta}_1) \sim t_{n-2}$ bajo $H_0$.

\vspace{0.1cm}
\textbf{Regla de decisi\'on:}
\begin{itemize}
  \item Rechazamos $H_0$ si $|t| > t_{\alpha/2, n-2}$ o si p-valor $< \alpha$
\end{itemize}

\vspace{0.1cm}
\begin{exampleblock}{Interpretaci\'on}
Si rechazamos $H_0$, concluimos que la pendiente es \textbf{estad\'isticamente diferente de cero}, es decir, existe una relaci\'on lineal significativa entre $X$ e $Y$.
\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{Aplicaci\'on en R: \texttt{summary(lm())}}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
...
\vspace{0.1cm}
{\small
\textbf{Residual standard error:} $s = 12.35$ (desviaci\'on t\'ipica de residuos)\\
\textbf{Multiple R-squared:} $R^2 = 0.6524$ (65.24\% de variabilidad explicada)\\
\textbf{F-statistic:} $F = 8481$, p-valor $< 2.2 \times 10^{-16}$ (prueba global)
}

\vspace{0.1cm}
\begin{alertblock}{Conclusi\'on}
\small Existe una relaci\'on lineal \textbf{muy significativa} entre el ingl\'es de Saber 11 y Saber Pro. Por cada punto adicional en Saber 11, esperamos 1.82 puntos m\'as en Saber Pro.
\end{alertblock}
\end{frame}
...
Una \textbf{variable dummy} es una variable binaria que toma valores 0 o 1 para representar categor\'ias.

\vspace{0.2cm}
\begin{exampleblock}{Ejemplo}
Queremos comparar el puntaje de ingl\'es entre IES privadas y p\'ublicas:
\[
\text{PRIVADA} = \begin{cases}
1 & \text{si IES privada} \\
0 & \text{si IES p\'ublica}
\end{cases}
\]
...
\begin{frame}{Variables dummy: modelo e interpretaci\'on}
\textbf{Modelo:}
\[
...
\textbf{Interpretaci\'on:}
\begin{itemize}
  \item $\beta_0$ = puntaje promedio en IES p\'ublicas (cuando PRIVADA = 0)
  \item $\beta_0 + \beta_1$ = puntaje promedio en IES privadas (cuando PRIVADA = 1)
  \item $\beta_1$ = diferencia promedio entre IES privadas y p\'ublicas
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Regresi\'on con dummy en R}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
...
La regresi\'on con una dummy es equivalente a una prueba t de dos muestras independientes:
...
\small El estad\'istico $t$ de la regresi\'on ($t = 12.67$) es id\'entico (en valor absoluto) al de la prueba t de dos muestras. La regresi\'on es una generalizaci\'on poderosa.
\end{block}
\end{frame}
...
\begin{frame}[fragile]{Ejemplo integrador: diagn\'ostico}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
...
\begin{frame}[fragile]{Ejemplo integrador: predicci\'on}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
...
\begin{itemize}
  \item Modelo de regresi\'on lineal simple: $Y = \beta_0 + \beta_1 X + \varepsilon$
  \item OLS minimiza $\sum e_i^2$ para estimar $\hat{\beta}_0$ y $\hat{\beta}_1$
  \item La l\'inea OLS pasa por $(\bar{X}, \bar{Y})$
  \item $R^2$ mide la proporci\'on de variabilidad explicada
...
\begin{itemize}
  ...
  \item Regresi\'on con dummy $\equiv$ t-test de dos muestras
\end{itemize}
\end{block}
\end{frame}

\begin{frame}{Pr\'oxima sesi\'on}
\begin{exampleblock}{Regresi\'on lineal m\'ultiple}
Incorporar varios predictores simult\'aneamente: por ejemplo, predecir su nota controlando por horas de estudio, screen time, y si trabajan.
\end{exampleblock}
\end{frame}
