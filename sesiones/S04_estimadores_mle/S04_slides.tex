...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Usualmente \alert{desconocidos} (¿cuál es la media verdadera de inglés de TODOS los estudiantes de Ciencias Econ\'omicas en Colombia?)
  \end{itemize}
...
\alert{Limitaci\'on}: un solo n\'umero no captura la incertidumbre de la estimaci\'on
\end{frame}

\begin{frame}{Estimaci\'on puntual: ejemplo cotidiano}
\begin{exampleblock}{Ejemplo: Spotify y la estimaci\'on}
Spotify estima tu ``perfil musical'' con un conjunto de par\'ametros. Cada canci\'on que escuchas es un dato. $\hat{\theta}$ es la mejor aproximaci\'on de tus gustos basada en esos datos.
\end{exampleblock}

\vspace{0.5em}
\textbf{Idea clave}: un estimador puntual nos da un solo n\'umero. Pero ?`qu\'e tan cerca est\'a del valor real? Para responder esa pregunta, necesitamos \alert{intervalos de confianza}.
\end{frame}
...
\begin{itemize}
  \item A medida que $n \to \infty$, el estimador converge al par\'ametro
  \item $\bar{x} \xrightarrow{p} \mu$ cuando $n \to \infty$
  \item Ejemplo: entre m\'as canciones le das ``like'' en Spotify, mejor estima tus gustos musicales
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Ejemplo: Estimación Puntual con Datos ICFES}
\textbf{Contexto}: Queremos estimar la media poblacional de \variable{MOD\_INGLES\_PUNT} para estudiantes de Ciencias Econ\'omicas en Colombia.
...
\alert{No}, pero probablemente est\'a cerca.
...
\vspace{0.5cm}
Estamos \alert{95\% confiados} de que $\mu$ est\'a en ese intervalo.

\vspace{0.3cm}
\small Ejemplo: ``el \emph{screen time} promedio del sal\'on est\'a entre 4.2 y 5.8 horas''.
\end{columns}
...
\begin{alertblock}{Interpretaci\'on CORRECTA}
``Si tom\'aramos 100 muestras aleatorias diferentes y construy\'eramos un IC al 95\% con cada una, esperar\'iamos que aproximadamente \alert{95 de esos intervalos} contengan el par\'ametro verdadero $\mu$.''
\end{alertblock}

\smallskip
\begin{exampleblock}{Interpretaci\'on alternativa correcta}
``Estamos 95\% confiados de que el \emph{screen time} promedio de los javerianos est\'a entre 4.2 y 5.8 horas.''
\end{exampleblock}

\smallskip
\begin{block}{Interpretaci\'on INCORRECTA}
...
\textbf{Componentes}:
\begin{itemize}
  \item $z_{\alpha/2}$ = valor cr\'itico de la distribuci\'on normal est\'andar
  \item $\frac{\sigma}{\sqrt{n}}$ = error est\'andar de $\bar{x}$
  \item Mayor confianza $(1-\alpha)$ $\Rightarrow$ mayor $z_{\alpha/2}$ $\Rightarrow$ mayor $E$
  \item Mayor $n$ $\Rightarrow$ menor $E$
\end{itemize}
\end{frame}

\begin{frame}{Trade-off: Confianza vs Precisi\'on}
\textbf{Trade-off}: Confianza vs Precisi\'on
\begin{itemize}
  \item M\'as confianza (ej.\ 99\%) $\Rightarrow$ intervalo m\'as ancho
  \item Menos confianza (ej.\ 90\%) $\Rightarrow$ intervalo m\'as estrecho
\end{itemize}

\begin{exampleblock}{Analog\'ia}
Predecir la hora exacta a la que un amigo llega a clase es dif\'icil. Pero decir ``llega entre 6:55 y 7:10'' es m\'as seguro. Entre m\'as amplio el rango, m\'as confianza, pero menos precisi\'on.
\end{exampleblock}
\end{frame}
...
\textbf{Escenario}: Supongamos que de censos anteriores sabemos que $\sigma = 18$ para \variable{MOD\_INGLES\_PUNT} en Ciencias Econ\'omicas. Tenemos una muestra de $n = 120$ estudiantes con $\bar{x} = 162.5$.

\vspace{0.2cm}
\textbf{Calcular IC al 95\%} -- paso 1: datos y valor crítico.
...
\begin{frame}[fragile]{Ejemplo Numérico: IC con $\sigma$ Conocida (cont.)}
\textbf{Paso 2}: Margen de error y límites del intervalo.
...
IC al 95\% para $\mu$: $(159.28, 165.72)$ puntos, con margen de error $E = 3.22$.
\end{exampleblock}
\end{frame}

\begin{frame}{Interpretaci\'on del Resultado}
\begin{exampleblock}{Resultado}
IC al 95\% para $\mu$: $(159.28, 165.72)$ puntos
\end{exampleblock}

\vspace{0.3cm}
\textbf{Interpretaci\'on correcta}:
\begin{itemize}
  \item Estamos 95\% confiados de que la media poblacional de \variable{MOD\_INGLES\_PUNT} para estudiantes de Ciencias Econ\'omicas est\'a entre 159.28 y 165.72 puntos.
  \item Si repiti\'eramos el muestreo 100 veces, aproximadamente 95 de los intervalos construidos contendr\'ian el verdadero valor de $\mu$.
\end{itemize}
...
\begin{itemize}
  ...
  \item ?`Es suficientemente preciso para nuestros prop\'ositos?
  \item Si necesitamos m\'as precisi\'on, podemos aumentar $n$
\end{itemize}
\end{frame}

\begin{frame}{El IC en la vida real}
\begin{alertblock}{Ejemplo: encuestas electorales}
Las encuestas electorales reportan ``el candidato A tiene 48\% $\pm$ 3\%''. Ese $\pm$ 3\% es el margen de error de un IC, igual al que acabamos de calcular.
\end{alertblock}

\vspace{0.5em}
\textbf{Otros ejemplos cotidianos de IC}:
\begin{itemize}
  \item Reportes de salud: ``el consumo promedio de cafe\'ina de universitarios colombianos es 180 $\pm$ 25 mg/d\'ia''
  \item Apps de delivery: ``tiempo estimado: 25--35 minutos'' (intervalo de confianza impl\'icito)
\end{itemize}
\end{frame}
...
\textbf{Situaci\'on real}: En la pr\'actica, \alert{casi nunca} conocemos $\sigma$.

\vspace{0.2cm}
\textbf{Soluci\'on}: Usar la desviaci\'on est\'andar muestral $s$ en lugar de $\sigma$.

\vspace{0.2cm}
\begin{block}{Consecuencia}
Al reemplazar $\sigma$ por $s$, introducimos una fuente adicional de variabilidad. El estad\'istico
\[
t = \frac{\bar{x} - \mu}{s/\sqrt{n}}
\]
ya NO sigue una distribuci\'on normal est\'andar, sino una \alert{distribuci\'on $t$ de Student} con $n-1$ grados de libertad.
\end{block}
\end{frame}

\begin{frame}{$\sigma$ Desconocida: analog\'ia}
\begin{exampleblock}{Analog\'ia}
Imagina que quieres estimar el promedio de horas de sue\~no de tu sal\'on, pero no conoces qu\'e tan dispersos son los datos ($\sigma$). Solo tienes $s$ de tu muestra. Esa incertidumbre extra es la que captura la distribuci\'on $t$.
\end{exampleblock}

\vspace{0.5em}
\textbf{Resumen}: Cuando $\sigma$ es desconocida, usamos $s$ y la distribuci\'on $t$ en lugar de la normal est\'andar $Z$. Esto produce intervalos m\'as anchos, especialmente con muestras peque\~nas.
\end{frame}
...
\begin{frame}[fragile]{Ejemplo: IC para la Media de MOD\_INGLES\_PUNT en Ciencias Econ\'omicas}
\textbf{Datos reales}: Muestra de $n = 120$ estudiantes de Ciencias Econ\'omicas.
...
\begin{frame}[fragile]{IC para la Media de MOD\_INGLES\_PUNT: cálculo}
\begin{lstlisting}
...
\vspace{0.2cm}
\textbf{Nota}: Usamos la distribución $t$ porque $\sigma$ es desconocida y la estimamos con $s$.
\end{frame}
...
\vspace{0.2cm}
\small\textbf{Ventaja}: \code{t.test()} también realiza una prueba de hipótesis (que veremos en sesiones futuras).
\end{frame}
...
\textbf{Motivaci\'on}: A veces queremos estimar la \alert{variabilidad} poblacional, no solo la media.

\vspace{0.3cm}
\textbf{Aplicaciones}:
\begin{itemize}
  \item Educaci\'on: ?`qu\'e tan heterog\'eneo es el rendimiento en ingl\'es entre estudiantes de Ciencias Econ\'omicas?
  \item Comercio internacional: volatilidad de la TRM para planear importaciones
  \item Log\'istica: variabilidad en tiempos de entrega de Rappi --- si la varianza es alta, no puedes confiar en la hora estimada
\end{itemize}
\end{frame}

\begin{frame}{Distribuci\'on muestral de $s^2$}
\begin{block}{Distribuci\'on muestral de $s^2$}
Si la poblaci\'on es normal, entonces:
\[
\frac{(n-1)s^2}{\sigma^2} \sim \chi^2(n-1)
\]
donde $\chi^2(n-1)$ es la distribuci\'on chi-cuadrado con $n-1$ grados de libertad.
\end{block}

\vspace{0.5em}
Este resultado nos permite construir intervalos de confianza para $\sigma^2$ usando la distribuci\'on $\chi^2$.
\end{frame}
...
\vspace{0.3cm}
\small
\textbf{Nota}: A diferencia de $z$ y $t$, la $\chi^2$ NO es simétrica; los valores críticos no son opuestos.
\end{frame}
...
\begin{frame}[fragile]{IC para la Varianza: resultado}
\begin{lstlisting}
cat("s^2 =", round(s2, 2))
...
\vspace{0.3cm}
\textbf{Nota}: El IC para $\sigma$ se obtiene tomando raíz cuadrada de los límites del IC para $\sigma^2$.
\end{frame}
...
ggplot(ic_deptos, aes(x = media, y = reorder(ESTU_DEPTO_RESIDE, media))) +
  ...
  labs(x = "Media de MOD_INGLES_PUNT (IC 95%)",
       ...
       subtitle = "Estudiantes de Ciencias Economicas y Administrativas") +
  theme_minimal()
\end{lstlisting}
\end{frame}
...
\begin{tikzpicture}[scale=0.85, every node/.style={transform shape},
  ...
  decision/.style={diamond, draw, fill=blue!20, text width=2.2cm, align=center, inner sep=0pt, font=\small},
  block/.style={rectangle, draw, fill=green!20, text width=2.8cm, align=center, rounded corners, minimum height=0.7cm, font=\small},
  line/.style={draw, -{Latex[length=2mm]}}
]

  \node [block] (inicio) {?`Qu\'e par\'ametro estimar?};

  \node [decision, below=0.5cm of inicio] (param) {$\mu$ o $\sigma^2$?};

  \node [decision, below left=0.8cm and 1.6cm of param] (sigma_conocida) {?`$\sigma$ conocida?};
  \node [block, below=0.6cm of sigma_conocida] (usa_z) {Usar dist.\ $z$};
  \node [block, right=1.6cm of usa_z] (usa_t) {Usar dist.\ $t$};

  \node [block, below right=0.8cm and 1.6cm of param] (usa_chi2) {Usar dist.\ $\chi^2$};
...
\begin{enumerate}
  \item Calcular IC al 95\% y 99\% para la media de \variable{MOD\_RAZONA\_CUANTITAT} en Ciencias Econ\'omicas
