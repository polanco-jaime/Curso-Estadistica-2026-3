...
\begin{itemize}
  \item Grupos completamente distintos (ej. p\'ublicas vs privadas)
  \item Sin relaci\'on entre observaciones de un grupo y otro
\end{itemize}

\vspace{0.3cm}
\textbf{Ahora}: Muestras \alert{dependientes (pareadas)}
\begin{itemize}
  \item Mismos sujetos medidos dos veces (antes/despu\'es)
  \item Sujetos emparejados naturalmente (hermanos, pares matched)
  \item Cada observaci\'on en el grupo 1 tiene una contraparte espec\'ifica en el grupo 2
\end{itemize}
...
\begin{itemize}
  ...
  \item Rendimiento antes vs despu\'es de una intervenci\'on
  \item Calificaci\'on de dos evaluadores sobre el \alert{mismo} trabajo
\end{itemize}
\end{frame}

\begin{frame}{Muestras Dependientes: ejemplo cotidiano}
\begin{exampleblock}{Screen time antes y despu\'es}
Medir tu \alert{screen time} ANTES y DESPU\'ES de instalar una app de bienestar digital (ej.\ Forest, One Sec). Cada persona es su propio control: se comparan sus dos mediciones.
\end{exampleblock}

\vspace{0.3cm}
\textbf{Caracter\'isticas clave}:
\begin{itemize}
  \item La misma persona se mide dos veces $\Rightarrow$ datos pareados
  \item Cada par $(x_{antes}, x_{despu\'es})$ est\'a vinculado
  \item Se analizan las \alert{diferencias} $d_i = x_{despu\'es} - x_{antes}$
\end{itemize}
\end{frame}
...
\]
\vspace{-0.3cm}
donde $\bar{d} = \frac{1}{n} \sum d_i$ (media de las diferencias), $s_d$ = DE de las diferencias, $n$ = n\'umero de pares, y $t \sim t(n-1)$ si $H_0$ es verdadera.
\end{block}
\end{frame}
...
\begin{frame}{Motivaci\'on: Comparar M\'as de Dos Grupos}
\textbf{Problema}: Queremos comparar ingl\'es entre 5 regiones de Colombia.

\vspace{0.3cm}
\textbf{Opci\'on incorrecta}: Hacer m\'ultiples pruebas $t$
\begin{itemize}
  \item Comparar Caribe vs Andina, Caribe vs Pac\'ifico, \ldots, etc.
  \item Con $k = 5$ grupos, tendr\'iamos $\binom{5}{2} = 10$ comparaciones
  \item Problema: \alert{inflaci\'on del error tipo I}
\end{itemize}

\vspace{0.3cm}
\begin{alertblock}{Inflaci\'on del error tipo I}
...
\begin{frame}{Motivaci\'on: Comparar M\'as de Dos Grupos (cont.)}
\textbf{Soluci\'on}: ANOVA (Analysis of Variance)
\begin{itemize}
  \item Compara las medias de \alert{tres o m\'as} grupos simult\'aneamente
  \item Controla el error tipo I global
\end{itemize}

\vspace{0.3cm}
\begin{exampleblock}{Ejemplo cotidiano}
?`Hay diferencia en el promedio de notas entre estudiantes que usan TikTok $<$1\,h, 1--3\,h y $>$3\,h al d\'ia? Tres grupos, una variable num\'erica $\Rightarrow$ ANOVA.
\end{exampleblock}
\end{frame}
...
\vspace{0.15cm}
\small
\textbf{Observaci\'on}: Efectos peque\~nos requieren muestras MUCHO m\'as grandes.
\end{frame}
...
\begin{frame}{?`Cu\'ando Usar Pruebas No Param\'etricas?}
\textbf{Situaciones}:
\begin{enumerate}
  \item \textbf{Datos ordinales} (ej.\ escala Likert: 1=malo, 2=regular, \ldots, 5=excelente)
  \item \textbf{Distribuciones muy sesgadas} o con outliers extremos
  \item \textbf{Muestras peque\~nas} donde no se puede verificar normalidad
  \item \textbf{Violaci\'on de supuestos} de las pruebas param\'etricas
\end{enumerate}
...
\begin{itemize}
  ...
  \item V\'alidas para datos ordinales
\end{itemize}

\vspace{0.3cm}
\textbf{Desventajas}:
\begin{itemize}
  \item \alert{Menos potentes} que las param\'etricas si los supuestos se cumplen
  \item Interpretaci\'on menos directa (trabajan con rangos, no con medias)
\end{itemize}
\end{frame}

\begin{frame}{Pruebas No Param\'etricas: ejemplo cotidiano}
\begin{exampleblock}{Ratings de apps}
Datos de rating en apps (1--5 estrellas) no siguen una distribuci\'on normal: son ordinales y discretos. Aqu\'i conviene usar Mann-Whitney o Kruskal-Wallis en lugar de $t$ o ANOVA.
\end{exampleblock}

\vspace{0.3cm}
\textbf{Regla pr\'actica}:
\begin{itemize}
  \item Si la variable es ordinal (escalas, rankings) $\Rightarrow$ no param\'etrica
  \item Si hay outliers extremos o asimetr\'ia fuerte $\Rightarrow$ considerar no param\'etrica
  \item Si $n$ es grande y la distribuci\'on es razonable $\Rightarrow$ param\'etrica (TLC)
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Ejemplo: Mann-Whitney en R -- Parte 1}
\begin{lstlisting}
...
\begin{frame}[fragile]{Ejemplo: Mann-Whitney en R -- Parte 2}
\begin{lstlisting}
print(resultado_mw)
...
\begin{frame}{Flujo de Decisi\'on}
\begin{center}
\begin{tikzpicture}[
  node distance=0.6cm and 1cm,
  decision/.style={diamond, draw, fill=blue!20, text width=1.8cm, align=center, inner sep=0pt, font=\footnotesize},
  block/.style={rectangle, draw, fill=green!20, text width=2.2cm, align=center, rounded corners, minimum height=0.4cm, font=\footnotesize},
  line/.style={draw, -{Latex[length=1.5mm]}}
]

  \node [block] (inicio) {?`Cu\'antos grupos?};
  \node [decision, below=0.4cm of inicio] (grupos) {1, 2, o $>$2?};
  \node [block, below left=0.6cm and 1.8cm of grupos] (un_grupo) {1 grupo: $t$};
  \node [decision, below=0.8cm of grupos] (dos_grupos) {?`Indep.?};
  \node [block, below left=0.5cm and 0.8cm of dos_grupos] (t_dos) {$t$ (2 muest.)};
  \node [block, below right=0.5cm and 0.8cm of dos_grupos] (t_pareada) {$t$ pareada};
  \node [block, below right=0.6cm and 1.8cm of grupos] (anova) {$>$2: ANOVA};
...
\end{center}
\small
Si los supuestos no se cumplen, cambiar a la prueba no param\'etrica correspondiente.
\end{frame}
