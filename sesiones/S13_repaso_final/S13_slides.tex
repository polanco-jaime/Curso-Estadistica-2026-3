...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw=javeriana, fill=javeriana!10, thick, minimum width=3.5cm, minimum height=1.1cm, rounded corners, align=center, font=\small},
...
\begin{frame}{Las 4 unidades del curso: conexiones}
\begin{block}{C\'omo se conectan las unidades}
\begin{itemize}
\item \textbf{U1 (Descriptiva + Probabilidad)}: Fundamento --- recuerden el screen time para la media y la desviaci\'on est\'andar
\item \textbf{U2 (IC)}: Cuantificar incertidumbre --- el margen de error de las encuestas electorales
\item \textbf{U3 (Pruebas)}: Tomar decisiones con datos --- ?`hay diferencia real o es solo ruido?
\item \textbf{U4 (Regresi\'on)}: Modelar relaciones --- ?`cu\'anto pesa cada factor en el puntaje de ingl\'es?
\end{itemize}
\end{block}

\vspace{0.2cm}
\small
\textcolor{javeriana}{Todo se integra}: en un an\'alisis completo usamos las 4 unidades (como la receta completa de S12).
\end{frame}
...
\begin{tikzpicture}[
  ...
  concept/.style={ellipse, draw=javeriana, fill=javeriana!10, thick, minimum width=1.5cm, minimum height=0.4cm, font=\scriptsize, align=center},
  arrow/.style={->, thick, javerianalight}
]
% Nivel 1: base
\node[concept] (datos) {Datos};
\node[concept, above right=0.4cm and 0.7cm of datos] (desc) {Descriptiva};
\node[concept, above left=0.4cm and 0.7cm of datos] (prob) {Probabilidad};

% Nivel 2: inferencia
\node[concept, above=0.7cm of desc] (muestreo) {Muestreo};
\node[concept, above right=0.4cm and 0.5cm of muestreo] (ic) {IC};
\node[concept, above left=0.4cm and 0.5cm of muestreo] (pruebas) {Pruebas};

% Nivel 3: modelado
\node[concept, above=0.7cm of ic] (regresion) {Regresión};
\node[concept, above=0.15cm of regresion] (diagnosticos) {Diagnósticos};

% Nivel 4: conclusiones
\node[concept, above=0.4cm of diagnosticos] (conclusiones) {Conclusiones};
...
\begin{frame}{U1: Medidas de tendencia central y dispersión}
\begin{block}{Medidas de tendencia central}
\begin{itemize}
\item \textbf{Media}: $\bar{x} = \frac{1}{n}\sum_{i=1}^{n}x_i$ (sensible a valores extremos --- recuerden: un millonario en el sal\'on sube el promedio de ingresos de todos)
\item \textbf{Mediana}: valor que divide los datos en 50\%-50\% (robusta)
\item \textbf{Moda}: valor m\'as frecuente
\end{itemize}
\end{block}
...
\begin{frame}{U1: Medidas de forma}
\begin{block}{Asimetría y curtosis}
\begin{itemize}
...
\begin{frame}{U1: Correlación de Pearson}
...
\begin{frame}{U1: Correlación de Spearman}
...
\begin{frame}{U1: Distribución normal}
...
\begin{frame}{U1: Teorema Central del Límite}
...
\textbf{Implicaci\'on}: Podemos hacer inferencia sobre $\mu$ incluso si $X$ no es normal (con $n$ grande). Recuerden: aunque el screen time individual de cada uno sea bien raro, el promedio de 100 estudiantes se comporta ``normal''.
\end{block}
\end{frame}

\begin{frame}{U1: Distribución $t$ de Student}
...
\begin{frame}{U1: Distribución $\chi^2$ (chi-cuadrado)}
...
\begin{frame}{U2: IC para la media ($\sigma$ conocida)}
...
\begin{frame}{U2: IC para la media ($\sigma$ desconocida)}
...
$\chi^2_{0.025, 49} = 70.22$, $\chi^2_{0.975, 49} = 31.55$.
IC: $\left[\frac{49 \cdot 349.69}{70.22}, \frac{49 \cdot 349.69}{31.55}\right] = [244.0, 543.5]$.
El IC para $\sigma$ es $[\sqrt{244.0}, \sqrt{543.5}] = [15.6, 23.3]$.
\end{exampleblock}
\end{frame}
...
\begin{exampleblock}{Ejemplo}
\small
$n = 200$, $x = 75$ alcanzan B1. $\hat{p} = 0.375$. IC 95\%:
$$0.375 \pm 1.96 \cdot \sqrt{\frac{0.375 \cdot 0.625}{200}} = 0.375 \pm 0.067 = [0.308, 0.442]$$
\end{exampleblock}
\end{frame}

\begin{frame}{U2: Tama\~no de muestra para $\mu$}
...
\begin{frame}{U2: Tamaño de muestra para $p$}
...
\begin{frame}{U2: Valores críticos comunes}
\textbf{Recordar}:
\begin{itemize}
...
\begin{frame}{U3: Pasos de una prueba de hipótesis}
\begin{block}{Pasos}
\begin{enumerate}
...
\begin{frame}{U3: Errores tipo I y II}
\begin{block}{Errores tipo I y II}
\begin{itemize}
\item \textbf{Error tipo I}: Rechazar $H_0$ cuando es verdadera (falso positivo). Probabilidad = $\alpha$. \\ \small Recuerden: como cuando el filtro de spam bloquea un email importante.
\item \textbf{Error tipo II}: No rechazar $H_0$ cuando es falsa (falso negativo). Probabilidad = $\beta$. \\ \small Como cuando el filtro deja pasar spam a la bandeja de entrada.
...
\begin{frame}{U3: Prueba t de una muestra --- ejemplo}
\begin{exampleblock}{Ejemplo}
...
\begin{frame}{U3: Prueba t de dos muestras (Welch)}
\begin{block}{Hipótesis}
...
\begin{frame}[fragile]{U3: Prueba t de Welch --- ejemplo}
...
\begin{frame}{U3: ANOVA de un factor}
\begin{block}{Hipótesis}
...
\small
$SS_{entre} = \sum_{i=1}^{k} n_i (\bar{x}_i - \bar{x})^2$ (entre grupos), \quad
$SS_{dentro} = \sum_{i=1}^{k} \sum_{j=1}^{n_i} (x_{ij} - \bar{x}_i)^2$ (dentro de grupos)
\end{block}
\end{frame}

\begin{frame}[fragile]{U3: ANOVA --- post-hoc}
...
\begin{frame}[fragile]{U3: Chi-cuadrado --- ejemplo}
\begin{exampleblock}{Ejemplo en R}
\begin{lstlisting}
...
\begin{frame}[fragile]{U3: No paramétricas --- ejemplo}
...
\begin{frame}{U3: d de Cohen (tamaño del efecto)}
...
donde:
\begin{itemize}
\item $Y_i$ = variable dependiente, $X_i$ = variable independiente
\item $\beta_0$ = intercepto, $\beta_1$ = pendiente (efecto de $X$ sobre $Y$)
\item $\varepsilon_i$ = error (ruido)
\end{itemize}
\end{block}

\begin{block}{Estimadores OLS}
$$\hat{\beta}_1 = \frac{\sum_{i=1}^{n}(X_i - \bar{X})(Y_i - \bar{Y})}{\sum_{i=1}^{n}(X_i - \bar{X})^2} = \frac{Cov(X, Y)}{Var(X)} \qquad \hat{\beta}_0 = \bar{Y} - \hat{\beta}_1 \bar{X}$$
\end{block}

\small
\textbf{Interpretaci\'on}: $\hat{\beta}_1$ es el cambio en $Y$ asociado con un cambio unitario en $X$.
\end{frame}

\begin{frame}{U4: Modelo de regresi\'on m\'ultiple}
\begin{block}{Modelo}
...
\begin{frame}{U4: Regresión múltiple --- ejemplo}
\begin{exampleblock}{Ejemplo}
...
\begin{frame}{U4: $R^2$ (coeficiente de determinación)}
\begin{block}{$R^2$}
...
\begin{frame}{U4: Prueba t para $\beta_j$}
...
\begin{frame}{U4: Prueba F global}
...
Modelo m\'ultiple: $\text{INGLES} = \beta_0 + \beta_1 \text{ESTRATO} + \beta_2 \text{PRIVADA} + \beta_3 \text{PUNT\_S11} + \varepsilon$ $\Rightarrow$ $\hat{\beta}_1 = 2.1$

El efecto se reduce porque sin controles, el estrato cargaba con el efecto de otras variables --- como culpar solo al Wi-Fi cuando el problema real es que ten\'ian 20 pesta\~nas abiertas.
\end{exampleblock}
\end{frame}

\begin{frame}{U4: Multicolinealidad}
...
\begin{frame}{U4: VIF (Variance Inflation Factor)}
\begin{block}{VIF}
...
\item \textbf{Cook's distance}: observaciones influyentes ($D_i > 4/n$) --- recuerden el ejemplo del estudiante que falt\'o a todo pero sac\'o 5.0 en el final

\item \textbf{VIF}: multicolinealidad ($VIF > 10$ $\Rightarrow$ considerar eliminar variable)
\end{enumerate}
\end{block}
\end{frame}
...
\begin{frame}{U4: Errores robustos --- recordar}
\textbf{Recordar}:
\begin{itemize}
...
\begin{frame}{Problema 1: Soluciones (parte 1)}
\begin{enumerate}
...
\begin{frame}{Problema 1: Soluciones (parte 2)}
\begin{enumerate}
\setcounter{enumi}{2}
...
Una muestra de $n = 100$ estudiantes de Ciencias Econ\'omicas tiene puntaje promedio de inglés $\bar{x} = 158.5$ con desviación estándar $s = 20.3$.
\end{exampleblock}
...
\begin{frame}{Problema 2: Soluciones}
\begin{enumerate}
...
\begin{frame}{Problema 3: Soluciones}
\begin{enumerate}
...
\begin{frame}{Problema 4: Soluciones}
\begin{enumerate}
...
\begin{frame}{Estrategias de estudio}
\begin{block}{Preparación para el examen}
\begin{enumerate}
...
\item \textbf{Estudiar en grupo}: Explica conceptos a tus compa\~neros (si puedes explic\'arselo a alguien por WhatsApp, lo entendiste).
...
\begin{frame}{Recursos de estudio}
...
\item Estadístico t (1, 2 muestras, pareada)
...
\item \textbf{Resultados} (6-8 págs): EDA, IC, pruebas, regresión, diagnósticos
\item \textbf{Conclusiones} (2-3 págs): Hallazgos, implicaciones, limitaciones
\item \textbf{Referencias} y \textbf{Anexos} (código R, tablas adicionales)
\end{enumerate}
\end{block}
\end{frame}
...
\begin{frame}{Consejos para el proyecto: errores comunes}
...
\begin{frame}{Consejos para el proyecto: buenas prácticas}
...
\item \textbf{Contextualizar}: Conectar resultados estadísticos con implicaciones para Ciencias Econ\'omicas
\end{itemize}
\end{exampleblock}
\end{frame}
...
\begin{frame}{Recapitulación: habilidades técnicas}
...
\begin{frame}{Recapitulación: habilidades conceptuales}
\begin{block}{Habilidades conceptuales}
\begin{itemize}
\item Pensar críticamente sobre datos (¿de dónde vienen? ¿qué sesgos hay?)
\item Distinguir correlaci\'on de causalidad (recuerden: que el helado y los ahogamientos suban juntos no significa que uno cause el otro)
...
\begin{frame}{M\'as all\'a del curso}
\begin{block}{?`C\'omo usar estad\'istica en tu carrera?}
\small
\begin{itemize}
\item \textbf{Ciencias Econ\'omicas y Administrativas}: An\'alisis de mercados, evaluaci\'on de pol\'iticas comerciales, estudios de competitividad
\item \textbf{Consultor\'ia}: Diagn\'osticos basados en datos, evaluaci\'on de impacto
\item \textbf{Pol\'itica p\'ublica}: Dise\~no y evaluaci\'on de programas sociales
\item \textbf{Investigaci\'on acad\'emica}: Tesis, art\'iculos, proyectos de grado
\end{itemize}
\end{block}
\end{frame}
...
\item \textbf{Pr\'actica}: Kaggle, datos abiertos (DANE, Banco Mundial)
\end{itemize}
\end{block}
\end{frame}
...
\small
\begin{itemize}
\item \textbf{Lunes 24 de noviembre}: 14:00-16:00 (oficina 305) --- Repaso de U2 (IC) y U3 (pruebas)
\item \textbf{Miércoles 26 de noviembre}: 14:00-16:00 (oficina 305) --- Repaso de U4 (regresión) y problemas tipo examen
\item \textbf{Jueves 26 de noviembre}: 10:00-12:00 (por Zoom) --- Sesión de preguntas abiertas (cualquier tema)
\end{itemize}
\end{alertblock}
