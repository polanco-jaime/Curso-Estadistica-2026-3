...
\begin{frame}{U2: Tamaño de muestra para $\mu$}
...
Modelo múltiple: $\text{INGLES} = \beta_0 + \beta_1 \text{ESTRATO} + \beta_2 \text{PRIVADA} + \beta_3 \text{PUNT\_S11} + \varepsilon$ $\Rightarrow$ $\hat{\beta}_1 = 2.1$

El efecto se reduce porque en el modelo simple capturaba TAMBIÉN el efecto de PRIVADA y PUNT\_S11.
\end{exampleblock}
\end{frame}
...
\item \textbf{Cook's distance}: observaciones influyentes ($D_i > 4/n$)
...
\item \textbf{Práctica}: Kaggle, datos abiertos (DANE, Banco Mundial)
\end{itemize}
\end{block}
\end{frame}
