...
\textbf{Lectura}: el 42\% de los estudiantes obtiene entre 140 y 159 puntos; el 64.5\% está por debajo de 160.
\end{frame}
...
\begin{block}{Definición}
Un \textbf{histograma} muestra la distribución de frecuencias de una variable continua mediante barras contiguas.
\end{block}
...
\vspace{0.3em}
\textbf{Nota}: estos rangos son guías orientativas, no reglas absolutas.
\end{frame}
...
\vspace{0.5em}
\textbf{Ejemplo}: la correlación entre consumo de helados y ahogamientos es alta, pero la variable confusora es la temperatura.
\end{frame}
...
Estrato (ordinal: 1, ..., 6) vs puntaje de inglés: Spearman es más apropiado que Pearson porque estrato es ordinal.
\end{exampleblock}
\end{frame}
