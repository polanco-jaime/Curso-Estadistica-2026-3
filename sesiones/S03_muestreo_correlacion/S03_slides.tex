...
\textbf{Notación}: mayúsculas (X, Y, Z) para la variable aleatoria; minúsculas (x, y, z) para valores específicos.

\begin{exampleblock}{Ejemplos}
\begin{itemize}
\item X = puntaje de ingl\'es en Saber Pro (valores entre 0 y 200)
\item Y = n\'umero de estudiantes que aprueban (valores 0, 1, 2, \ldots, n)
\item Z = horas diarias de \emph{screen time} en el celular (valores $\geq 0$)
\end{itemize}
\end{exampleblock}

\textbf{Tipos}:
\begin{itemize}
\item \textbf{Discreta}: valores separados, contables (ej: número de aprobados)
\item \textbf{Continua}: cualquier valor en un intervalo (ej: puntaje, altura, tiempo)
\end{itemize}
\end{frame}
...
\begin{frame}{Par\'ametros vs estad\'isticos}
\alert{Nota}: $\mu$ y $\sigma$ son \textbf{par\'ametros poblacionales} (fijos, desconocidos). $\bar{x}$ y $s$ son \textbf{estad\'isticos muestrales} (variables, observables).

\vspace{0.5em}
\begin{exampleblock}{Ejemplo: puntaje de ingl\'es como variable aleatoria}
Puntaje de ingl\'es Saber Pro para Ciencias Econ\'omicas como variable aleatoria continua X. De la misma forma, el \emph{screen time} diario de cada estudiante del sal\'on es una variable aleatoria continua.
\end{exampleblock}

\textbf{Interpretaci\'on probabil\'istica}:
\begin{itemize}
\item X toma valores entre 0 y 200, con media $\mu$ y desviaci\'on est\'andar $\sigma$
\item $P(X \leq 150)$ = proporci\'on con puntaje $\leq$ 150
\item $P(160 \leq X \leq 180)$ = proporci\'on entre 160 y 180
\end{itemize}
\end{frame}

\begin{frame}{Par\'ametros vs estad\'isticos (cont.)}
\textbf{Relaci\'on con estad\'istica descriptiva}:
\begin{itemize}
\item \alert{Toda la poblaci\'on}: $\mu$ y $\sigma$ se calculan directamente
\item \alert{Muestra}: estimamos $\mu$ con $\bar{x}$ y $\sigma$ con $s$
\end{itemize}

\alert{Pregunta clave}: ?`Cu\'al es la distribuci\'on de X? Esto determina c\'omo calculamos probabilidades.
\end{frame}
...
\begin{frame}{La distribuci\'on normal}
\begin{block}{Definici\'on}
La \textbf{distribuci\'on normal} (o gaussiana) es la distribuci\'on de probabilidad continua m\'as importante en estad\'istica.
\end{block}

\textbf{Funci\'on de densidad}:
\[
f(x) = \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}, \quad -\infty < x < \infty
\]

\textbf{Notaci\'on}: $X \sim N(\mu, \sigma^2)$ (X sigue una distribuci\'on normal con media $\mu$ y varianza $\sigma^2$)

\vspace{0.3em}
\textbf{Par\'ametros}:
\begin{itemize}
\item $\mu$: media (centro de la distribuci\'on) \quad $\sigma$: desviaci\'on est\'andar (dispersi\'on) \quad $\sigma^2$: varianza
\end{itemize}
\end{frame}

\begin{frame}{La distribuci\'on normal: ejemplo e intuici\'on}
\begin{exampleblock}{Ejemplo cotidiano}
El \emph{screen time} diario de los estudiantes del sal\'on sigue aproximadamente una campana de Gauss: la mayor\'ia usa el celular entre 4 y 6 horas, y muy pocos est\'an en los extremos (menos de 1 h o m\'as de 9 h).
\end{exampleblock}

\vspace{0.5em}
\alert{Importante}: La normal est\'a completamente determinada por $\mu$ y $\sigma$. Conociendo estos dos par\'ametros, podemos calcular cualquier probabilidad.
\end{frame}
...
\begin{block}{Definición}
La \textbf{distribución normal estándar} es una normal con $\mu = 0$ y $\sigma = 1$: \ $Z \sim N(0, 1)$
\end{block}

\textbf{Funci\'on de densidad}: $f(z) = \frac{1}{\sqrt{2\pi}} e^{-z^2/2}$

\textbf{Estandarizaci\'on}: Cualquier $X \sim N(\mu, \sigma^2)$ se transforma a:
\[
Z = \frac{X - \mu}{\sigma} \sim N(0, 1)
\]

\begin{alertblock}{?`Por qu\'e es \'util?}
\begin{itemize}
\item Todas las probabilidades se calculan con una sola tabla (tabla Z)
\item Permite comparar variables en escalas distintas (ej: tu puntaje Saber Pro vs tu n\'umero de seguidores en Instagram)
\item Simplifica c\'alculos te\'oricos
\end{itemize}
\end{alertblock}
\end{frame}
...
$X$ = puntaje de inglés Saber Pro Ciencias Econ\'omicas $\sim N(155, 20^2)$.
\end{exampleblock}

\textbf{a)} ¿Proporción con puntaje $\leq 150$?
\[
P(X \leq 150) = P\!\left(Z \leq \frac{150 - 155}{20}\right) = P(Z \leq -0.25) = 0.4013
\]
\alert{Respuesta}: Aproximadamente 40\% obtienen $\leq$ 150.

\vspace{0.3em}
\textbf{b)} ¿Proporción que supera 170?
\[
...
\textbf{c)} ¿Proporción entre 145 y 165?
\[
...
\begin{axis}[
  ...
  xticklabels={145, 155, 165}, height=3.5cm, width=9cm
]
\addplot[very thick, javeriana] {exp(-x^2/2)/sqrt(2*pi)};
...
\begin{block}{Definición}
Si $Z_1, \ldots, Z_k$ son independientes $N(0,1)$, entonces
$\chi^2 = Z_1^2 + \cdots + Z_k^2 \sim \chi^2(k)$.
\end{block}
...
\item Sesgada a la derecha
\item $E(\chi^2) = k$, \ $\text{Var}(\chi^2) = 2k$
\item Para $k$ grande, se aproxima a la normal
\end{itemize}
...
\item Independencia en tablas de contingencia
\end{itemize}
\end{columns}
\end{frame}
...
\item $E(t) = 0$ (si $k > 1$)
\item $\text{Var}(t) = k/(k-2)$ (si $k > 2$)
\item $t(k) \to N(0,1)$ cuando $k \to \infty$
\item Para $k \geq 30$, $\approx$ normal
\end{itemize}

\column{0.44\textwidth}
\textbf{Uso principal}:
\begin{itemize}
\item Inferencia sobre medias cuando $\sigma$ es desconocida:
\[
\frac{\bar{X} - \mu}{s/\sqrt{n}} \sim t(n-1)
\]
\item ICs y pruebas de hipótesis para medias
\end{itemize}
\end{columns}
\end{frame}

\begin{frame}{?`De d\'onde viene la distribuci\'on t?}
\textbf{Motivaci\'on hist\'orica}:

Cuando queremos inferir sobre la media poblacional $\mu$:
\begin{itemize}
\item Si conocemos $\sigma$: $\frac{\bar{X} - \mu}{\sigma/\sqrt{n}} \sim N(0,1)$
\item Pero en la pr\'actica, $\sigma$ es desconocido, usamos $s$
\item Reemplazar $\sigma$ por $s$ introduce incertidumbre adicional
\item William Gosset (seud\'onimo ``Student'') descubri\'o en 1908 que:
\end{itemize}
...
\textbf{Consecuencias}:
\begin{itemize}
\item Para muestras peque\~nas ($n < 30$), la distribuci\'on t tiene colas m\'as pesadas
\item Intervalos de confianza son m\'as anchos (reflejan mayor incertidumbre)
\item A medida que $n$ crece, $s \to \sigma$ y $t \to N(0,1)$
\end{itemize}
\end{frame}

\begin{frame}{Distribuci\'on t: muestra peque\~na vs grande}
\begin{alertblock}{Ejemplo: muestra peque\~na vs grande}
Si le preguntas a 5 amigos cu\'antas horas de Netflix ven, la variabilidad es alta y usas $t$. Si encuestas a 200 estudiantes, $s \approx \sigma$ y la $t$ se parece a la normal.
\end{alertblock}
...
\textbf{Propiedades}:
\begin{itemize}
\item Solo valores positivos ($F \geq 0$)
\item Asim\'etrica (sesgada a la derecha)
\item Depende de dos par\'ametros: $k_1$ (gl numerador) y $k_2$ (gl denominador)
\end{itemize}
...
\textbf{Población}
\begin{itemize}
\item Conjunto completo de elementos
\item Parámetros (fijos, desconocidos):
  \begin{itemize}
  \item Media: $\mu$
  \item Desv.\ estándar: $\sigma$
  \item Proporción: $p$
  \end{itemize}
...
\begin{frame}{Poblaci\'on vs muestra: ejemplo}
\begin{exampleblock}{Ejemplo concreto}
\begin{itemize}
\item \textbf{Poblaci\'on}: todos los estudiantes de Ciencias Econ\'omicas en Colombia ($N \approx 50{,}000$)
\item \textbf{Muestra}: 500 estudiantes seleccionados aleatoriamente de la base ICFES
\item Estimamos $\mu$ (puntaje medio poblacional) con $\bar{x}$ (puntaje medio muestral)
\end{itemize}
\end{exampleblock}

\begin{alertblock}{Analog\'ia}
{\small Es como encuestar a 30 compa\~neros del sal\'on sobre cu\'antas horas de Netflix ven al mes, en lugar de preguntarle a toda la universidad.}
\end{alertblock}

\begin{center}
\begin{tikzpicture}[scale=0.85, every node/.style={font=\footnotesize},
  box/.style={draw, rounded corners, minimum width=2.6cm, minimum height=1cm, align=center}]
\node[box, fill=javeriana!10] (pop) {Poblaci\'on\\$N \approx 50{,}000$, $\mu$};
\node[box, fill=javerianalight!15, right=2.5cm of pop] (samp) {Muestra\\$n = 500$, $\bar{x}$};
\draw[-{Stealth}, thick] (pop) -- node[above] {muestreo} (samp);
\draw[-{Stealth}, thick, dashed] (samp.south) -- ++(0,-0.4) -| node[below, pos=0.25] {inferencia} (pop.south);
\end{tikzpicture}
\end{center}
\end{frame}
...
\begin{block}{Definici\'on}
En el \textbf{muestreo aleatorio simple} (MAS), cada elemento de la poblaci\'on tiene la misma probabilidad de ser seleccionado, y cada muestra del mismo tama\~no tiene igual probabilidad.
\end{block}

\textbf{Supuestos clave}:
\begin{enumerate}
\item Las observaciones son \alert{independientes} entre s\'i
\item Cada observaci\'on proviene de la \alert{misma distribuci\'on} (poblaci\'on)
\item Notaci\'on: $X_1, X_2, \ldots, X_n$ son i.i.d.\ (independientes e id\'enticamente distribuidas)
\end{enumerate}

\vspace{0.5em}
\textbf{Consecuencias}:
\begin{itemize}
\item Evita sesgos de selecci\'on
\item Permite generalizar de la muestra a la poblaci\'on
\item Fundamento matem\'atico de la inferencia estad\'istica
\end{itemize}
\end{frame}

\begin{frame}{Muestreo aleatorio simple: ejemplo}
\begin{exampleblock}{Ejemplo}
Quieres saber cu\'antas horas semanales dedican los javerianos a TikTok. Si solo encuestas a tus amigos cercanos (sesgo de selecci\'on), el resultado no representa a toda la universidad. El MAS evita ese problema.
\end{exampleblock}

\vspace{0.5em}
\alert{En la pr\'actica}: muestreo aleatorio perfecto es dif\'icil. A veces usamos muestreo estratificado, por conglomerados, etc., pero MAS es el punto de partida te\'orico.
\end{frame}
...
\begin{enumerate}
\item \alert{$\bar{X}$ es menos variable que $X$}: \ $\text{DE}(\bar{X}) = \sigma/\sqrt{n} < \sigma$
\item \alert{SE disminuye con $\sqrt{n}$, no con $n$}: para reducir SE a la mitad, se necesitan $4\times$ más observaciones
\item \alert{Más grande la muestra, más precisa la estimación}:
  \begin{itemize}
  \item $n = 25$: $\text{SE} = \sigma/5$; \quad $n = 100$: $\text{SE} = \sigma/10$; \quad $n = 400$: $\text{SE} = \sigma/20$
  \end{itemize}
\end{enumerate}

\begin{exampleblock}{Ejemplo}
Si $\sigma = 20$ puntos y $n=100$: $\text{SE} = 20/10 = 2$ puntos.\\
La media muestral t\'ipicamente difiere de $\mu$ en $\approx 2$ puntos.

\smallskip
Encuestar a 100 compa\~neros sobre su \emph{screen time} da una estimaci\'on bastante precisa del promedio de toda la universidad.
\end{exampleblock}
\end{frame}
...
\begin{frame}{Teorema Central del L\'imite (TCL)}
\begin{block}{Teorema Central del L\'imite}
Si $X_1, X_2, \ldots, X_n$ es una muestra aleatoria de una poblaci\'on con media $\mu$ y desviaci\'on est\'andar $\sigma$ (finitos), entonces para $n$ suficientemente grande:
\[
\bar{X} \sim N\!\left(\mu, \frac{\sigma^2}{n}\right) \quad \text{aprox.} \qquad \Longleftrightarrow \qquad Z = \frac{\bar{X} - \mu}{\sigma/\sqrt{n}} \sim N(0, 1) \quad \text{aprox.}
\]
\end{block}
...
\textbf{Mensajes clave}:
\begin{enumerate}
\item \alert{No importa la forma de la distribuci\'on original}: sesgada, bimodal, uniforme, etc.
\item \alert{Para $n$ grande, $\bar{X}$ es aproximadamente normal}
\end{enumerate}
\end{frame}

\begin{frame}{TCL: ejemplo con TikTok}
\begin{exampleblock}{Ejemplo: sesiones de TikTok}
El tiempo por sesi\'on de TikTok NO es normal (muchas sesiones cortas, pocas muy largas). Pero si promedias el tiempo de 30 sesiones y repites muchas veces, esos promedios s\'i forman una campana. Eso es el TCL.
\end{exampleblock}

\vspace{0.5em}
\textbf{Intuici\'on}: No importa que la distribuci\'on individual sea rara. Al promediar muchas observaciones, el resultado siempre converge a una forma de campana.
\end{frame}

\begin{frame}{TCL: ¿cuándo es $n$ ``suficientemente grande''?}
\alert{Depende de la forma de la distribución original}:
\begin{itemize}
\item Población normal: $\bar{X}$ es \textbf{exactamente} normal para cualquier $n$
\item Población simétrica: $n \approx 15$ suele ser suficiente
\item Población muy sesgada: puede necesitarse $n \geq 50$ o más
\item Regla práctica general: $n \geq 30$
\end{itemize}
...
\begin{tabular}{lc}
\toprule
\textbf{Forma de la población} & \textbf{$n$ mínimo aprox.} \\
\midrule
Normal & cualquier $n$ \\
Simétrica & $\sim 15$ \\
Moderadamente sesgada & $\sim 30$ \\
Muy sesgada & $\geq 50$ \\
\bottomrule
\end{tabular}
}
...
\begin{frame}{?`Por qu\'e el TCL es tan importante?}
\begin{alertblock}{El TCL es el teorema m\'as importante de la estad\'istica}
\end{alertblock}
...
\item \textbf{Fundamento de la inferencia}:
  ...
  \item Pruebas de hip\'otesis sobre medias
  \item Tama\~no de muestra para precisi\'on deseada
  \end{itemize}
\item \textbf{Universalidad}:
  \begin{itemize}
  \item Aplica a cualquier poblaci\'on (con media y varianza finitas)
  \item No necesitamos conocer la distribuci\'on poblacional
  \end{itemize}
\item \textbf{Justifica el uso de la distribuci\'on normal}:
  \begin{itemize}
  \item Muchos fen\'omenos naturales y sociales son sumas o promedios
...
\begin{frame}{El TCL en la vida real}
\alert{Sin el TCL, la inferencia estad\'istica ser\'ia much\'isimo m\'as complicada.}

\begin{exampleblock}{Ejemplo: Spotify y YouTube}
Los algoritmos de recomendaci\'on de Spotify y YouTube promedian miles de interacciones de usuarios. Gracias al TCL, esos promedios se comportan como normales, lo que permite calcular intervalos de confianza y tomar decisiones.
\end{exampleblock}
\end{frame}
...
\begin{block}{Definición}
Un \textbf{estimador} es una fórmula para aproximar un parámetro poblacional desconocido a partir de datos muestrales.
\end{block}
...
\begin{itemize}
\item $\hat{\mu} = \bar{X} = \frac{1}{n}\sum X_i$ \quad (ej: promedio de \emph{screen time} de 50 compa\~neros)
\item $\hat{\sigma}^2 = s^2 = \frac{1}{n-1}\sum(X_i - \bar{X})^2$
\item $\hat{p} = X/n$ \ (ej: proporci\'on de compa\~neros que usan Duolingo)
\end{itemize}
\end{exampleblock}
...
\begin{block}{Definición}
Un estimador $\hat{\theta}$ es \textbf{insesgado} si $E(\hat{\theta}) = \theta$.
En promedio, el estimador acierta al valor verdadero.
\end{block}
...
\item $\frac{1}{n}\sum(X_i-\bar{X})^2$ es \alert{sesgado}: $E = \frac{n-1}{n}\sigma^2 < \sigma^2$ \ (por eso usamos $n-1$)
\end{itemize}
\end{exampleblock}
...
\begin{itemize}
\item Menor varianza = estimaciones m\'as consistentes entre muestras
\item Menor varianza = intervalos de confianza m\'as estrechos
\end{itemize}

\vspace{0.3em}
\begin{exampleblock}{Ejemplo}
Para estimar $\mu$ de una distribuci\'on normal:
\begin{itemize}
...
\begin{block}{Definición}
$\hat{\theta}_n$ es \textbf{consistente} si
$\hat{\theta}_n \xrightarrow{P} \theta$ cuando $n \to \infty$.
\end{block}

\textbf{Interpretación}: con más datos, el estimador se acerca al valor verdadero.

\textbf{Condición suficiente}: $\lim_{n \to \infty} E(\hat{\theta}_n) = \theta$ \ y \ $\lim_{n \to \infty} \text{Var}(\hat{\theta}_n) = 0$

\begin{exampleblock}{Ejemplos}
\begin{itemize}
\item $\bar{X}$: $E(\bar{X}) = \mu$ y $\text{Var}(\bar{X}) = \sigma^2/n \to 0$ \quad (consistente)
...
\toprule
\textbf{Propiedad} & \textbf{Significado} & \textbf{Ejemplo} \\
\midrule
Insesgamiento & $E(\hat{\theta}) = \theta$ & $\bar{X}$, $s^2$, $\hat{p}$ \\
Eficiencia & Menor varianza (más preciso) & $\bar{X}$ vs mediana \\
Consistencia & $\hat{\theta}_n \to \theta$ cuando $n\to\infty$ & $\bar{X}$, $s^2$, $\hat{p}$ \\
\bottomrule
\end{tabular}
\end{table}
}
