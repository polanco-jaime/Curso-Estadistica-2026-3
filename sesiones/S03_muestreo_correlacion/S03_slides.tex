...
Puntaje de ingl\'es Saber Pro para NI como variable aleatoria continua X. De la misma forma, el \emph{screen time} diario de cada estudiante del sal\'on es una variable aleatoria continua.
\end{exampleblock}
...
$X$ = puntaje de inglés Saber Pro NI $\sim N(155, 20^2)$.
\end{exampleblock}
...
\begin{itemize}
\item \textbf{Poblaci\'on}: todos los estudiantes de NI en Colombia ($N \approx 50{,}000$)
...
Es como hacer una encuesta a 30 compa\~neros del sal\'on sobre cu\'antas horas de Netflix ven al mes, en lugar de preguntarle a toda la universidad.
\end{alertblock}
...
\node[box, fill=javeriana!10] (pop) {Poblaci\'on\\$N \approx 50{,}000$\\Par\'ametro: $\mu$};
\node[box, fill=javerianalight!15, right=3cm of pop] (samp) {Muestra\\$n = 500$\\Estad\'istico: $\bar{x}$};
\draw[-{Stealth}, thick] (pop) -- node[above] {muestreo} (samp);
\draw[-{Stealth}, thick, dashed] (samp.south) -- ++(0,-0.5) -| node[below, pos=0.25] {inferencia} (pop.south);
\end{tikzpicture}
\end{center}
\end{frame}
