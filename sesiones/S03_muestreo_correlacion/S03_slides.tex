...
\item Y = número de estudiantes que aprueban (valores 0, 1, 2, \ldots, n)
...
\begin{frame}{Parámetros vs estadísticos}
...
\vspace{0.5em}
\begin{exampleblock}{Ejemplo: puntaje de inglés como variable aleatoria}
Puntaje de inglés Saber Pro para NI como variable aleatoria continua X.
\end{exampleblock}

\textbf{Interpretación probabilística}:
\begin{itemize}
\item X toma valores entre 0 y 200, con media $\mu$ y desviación estándar $\sigma$
\item $P(X \leq 150)$ = proporción con puntaje $\leq$ 150
\item $P(160 \leq X \leq 180)$ = proporción entre 160 y 180
\end{itemize}

\textbf{Relación con estadística descriptiva}:
\begin{itemize}
\item \alert{Toda la población}: $\mu$ y $\sigma$ se calculan directamente
\item \alert{Muestra}: estimamos $\mu$ con $\bar{x}$ y $\sigma$ con $s$
\end{itemize}

\alert{Pregunta clave}: ¿Cuál es la distribución de X? Esto determina cómo calculamos probabilidades.
\end{frame}
...
\textbf{Función de densidad}: \ $f(z) = \frac{1}{\sqrt{2\pi}} e^{-z^2/2}$

\vspace{0.3em}
\textbf{Estandarización}: Cualquier $X \sim N(\mu, \sigma^2)$ se transforma a:
\[
...
\begin{frame}{Población vs muestra: ejemplo}
\begin{exampleblock}{Ejemplo concreto}
\begin{itemize}
\item \textbf{Población}: todos los estudiantes de NI en Colombia ($N \approx 50{,}000$)
...
\node[box, fill=javeriana!10] (pop) {Población\\$N \approx 50{,}000$\\Parámetro: $\mu$};
\node[box, fill=javerianalight!15, right=3cm of pop] (samp) {Muestra\\$n = 500$\\Estadístico: $\bar{x}$};
...
La media muestral típicamente difiere de $\mu$ en $\approx 2$ puntos.
\end{exampleblock}
\end{frame}
...
Si $X_1, X_2, \ldots, X_n$ es una muestra aleatoria de una población con media $\mu$ y desviación estándar $\sigma$ (finitos), entonces para $n$ suficientemente grande:
\[
...
\textbf{Mensajes clave}:
\begin{enumerate}
\item \alert{No importa la forma de la distribución original}: sesgada, bimodal, uniforme, etc.
...
\begin{itemize}
\item $\hat{\mu} = \bar{X} = \frac{1}{n}\sum X_i$
\item $\hat{\sigma}^2 = s^2 = \frac{1}{n-1}\sum(X_i - \bar{X})^2$
\item $\hat{p} = X/n$ \ (donde $X$ = número de éxitos)
\end{itemize}
\end{exampleblock}
